...
class Manager extends events_1.EventEmitter {
    ...
    constructor(options) {
        ...
        this.options = Object.assign({ plugins: [], nodes: [{ identifier: "default", host: "localhost" }], autoPlay: true, clientName: "erela.js", defaultSearchPlatform: "youtube" }, options);
    ...
    search(query, requester) {
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            ...
            const sources = {
                ...
                youtube: "yt",
                youtubemusic: "ytm",
                yandex: "ym", // Only works with https://github.com/davidffa/lavalink/releases
            };
    ...
    updateVoiceState(data) {
        ...
        if (data.t === "VOICE_SERVER_UPDATE") {
            ...
            state.event = Object.assign({}, data.d);
            if (state.event.endpoint.startsWith("us") || state.event.endpoint.startsWith("brazil")) {
                if (player.node.options.region && player.node.options.region !== "usa") {
                    const usaNodes = this.nodes.filter(node => node.options.region === "usa" && node.connected);
                    if (usaNodes) {
                        player.voiceState = state;
                        player.moveNode(usaNodes.sort((a, b) => a.stats.players - b.stats.players).first());
                        return;
                    }
                }
            }
            else {
                if (player.node.options.region && player.node.options.region !== "europe") {
                    const europeNodes = this.nodes.filter(node => node.options.region === "europe" && node.connected);
                    if (europeNodes) {
                        player.voiceState = state;
                        player.moveNode(europeNodes.sort((a, b) => a.stats.players - b.stats.players).first());
                        return;
                    }
                }
            }
