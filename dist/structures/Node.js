...
class Node {
    ...
    constructor(options) {
        ...
        check(options);
        this.options = Object.assign({ port: 2333, password: "youshallnotpass", secure: false, retryAmount: 5, retryDelay: 30e3, configureResuming: true }, options);
    ...
    connect() {
        ...
        this.socket.on("upgrade", this.upgrade.bind(this));
    ...
    fetchVersions() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                this.versions = yield this.makeRequest('versions');
            }
            catch (_a) {
                this.versions = null;
            }
    ...
    /** Gets Node socket latency in milliseconds */
            ...
            return new Promise((resolve, reject) => {
                ...
                const pong = (node) => {
                    if (node !== this)
                        return;
                    resolve(Date.now() - t1);
                    this.manager.removeListener("nodePong", pong);
                    clearTimeout(rejectTimeout);
                };
                this.manager.on("nodePong", pong);
                this.send({ op: "ping" });
            });
        });
    }
    ...
    setupResuming() {
        this.resumeKey = Math.random().toString(18);
        this.send({
    ...
    upgrade({ headers }) {
        if (this.resumeKey && !headers["Session-Resumed"])
            delete this.resumeKey;
    }
    open() {
        if (this.reconnectTimeout)
            clearTimeout(this.reconnectTimeout);
        if (this.options.configureResuming && !this.resumeKey)
            this.setupResuming();
        if (!this.versions)
            this.fetchVersions();
    ...
    close(code, reason) {
        const newNode = this.manager.nodes.filter(node => node.connected).sort((a, b) => a.stats.players - b.stats.players).first();
        if (newNode) {
            for (const player of this.manager.players.filter(p => p.node === this).values()) {
    ...
    message(d) {
        ...
        switch (payload.op) {
            ...
            case "event":
                ...
                break;
            case "pong":
                this.manager.emit("nodePong", this);
                break;
            default:
                this.manager.emit("nodeWarn", this, new Error(`Unexpected op "${payload.op}" with data: ${payload}`));
                return;
        }
    }
    handleEvent(payload) {
